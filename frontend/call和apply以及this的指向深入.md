...
![image](images/call和apply以及this的指向深入/1.png)  
（2）闭包的执行上下文创建过程
...
![image](images/call和apply以及this的指向深入/2.png)  
![image](images/call和apply以及this的指向深入/3.png)  
通过闭包的作用域链可以发现，innerFoo在全局中保留了一个引用，导致未被回收。  
（3）一个执行上下文的结构如下：  
![image](images/call和apply以及this的指向深入/4.png)  
